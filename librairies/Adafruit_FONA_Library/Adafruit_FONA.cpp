...
boolean Adafruit_FONA::readSMSPB(uint8_t i, char *smsbuff,
uint16_t maxlen, uint16_t *readlen) {
	// text mode
	if (! sendCheckReply(F("AT+CMGF=1"), F("OK"))) return false;
...
boolean Adafruit_FONA::getSMSSenderPB(uint8_t i, char *sender, int senderlen) {
	// Ensure text mode and all text mode parameters are sent.
	if (! sendCheckReply(F("AT+CMGF=1"), F("OK"))) return false;
	if (! sendCheckReply(F("AT+CSDH=1"), F("OK"))) return false;
	// Send command to retrieve SMS message and parse a line of response.
	...
	boolean result = parseReplyQuoted(F("+CMGR:"), sender, senderlen, ',', 2);
	if (strlen(sender) == 0){
		result = false;
	}
	// Drop any remaining data from the response.
...
boolean Adafruit_FONA::isSMSSenderPB(uint8_t i) {
	// Ensure text mode and all text mode parameters are sent.
	if (! sendCheckReply(F("AT+CMGF=1"), F("OK"))) return false;
	if (! sendCheckReply(F("AT+CSDH=1"), F("OK"))) return false;
	char Sender[255];
...
boolean Adafruit_FONA::deleteAllSMS(uint8_t delflag) {
	...
	return sendCheckReply(sendbuff, "OK", 2000);
	//AT commands manual announce 5s and 25s of max rresp. time for respectively deleting
